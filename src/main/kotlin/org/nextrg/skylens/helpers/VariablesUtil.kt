...
object VariablesUtil {
    ...
    /**
     * Returns the (scaled) center from an element with padding allowing for subpixel movement, requires float x1/x2 and y1/y2
     */
    @JvmStatic
    fun getFloatCenter(state: SpecialGuiElementRenderState, fx: Vector2f, fy: Vector2f, scale: Float): Vector2f {
        val x0 = state.x1(); val x1 = state.x2()
        val y0 = state.y1(); val y1 = state.y2()
