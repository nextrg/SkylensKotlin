...
object PressureDisplay {
    ...
    fun getPosition(): Pair<Float, Float> {
        ...
        fun map(value: Float): Float = (120f * (value - 0.5f)) * ModConfig.pressureDisplayScale
        var offsetX = map(ax)
        var offsetY = map(ay)
    ...
    private fun draw(guiGraphics: GuiGraphics, x: Float, y: Float, value: Float) {
        ...
        drawMarkerLines(guiGraphics, x, meterY, withAlpha(color2, lineTransparency))
        ...
        drawLine(guiGraphics, x, meterY, value, lineRadius, withAlpha(color1, 35), 1f, 0.25f, 1f, 0)
    ...
    private fun drawMarkerLines(guiGraphics: GuiGraphics, x: Float, y: Float, color: Int) {
        ...
        for (i in 0..steps) {
            ...
            drawLine(guiGraphics, x, y, degreesToRadians(135f) - degreesToRadians((i * 10 * 27/steps).toFloat()), 10f, withAlpha(lineColor, 60), 1f, boldness, 0f, 0)
